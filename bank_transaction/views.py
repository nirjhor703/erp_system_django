...
def product_search(request):
    ...
    sql = """
            ...
            JOIN transaction__groupes tg 
                ON t.groupe_id = tg.id

            JOIN transaction__main__heads tmh 
                ON tmh.id = tg.tran_groupe_type

            LEFT JOIN item__manufacturers m 
                ON t.manufacturer_id = m.id

            LEFT JOIN item__forms f 
                ON t.form_id = f.id

            LEFT JOIN item__categories c 
                ON t.category_id = c.id

            WHERE 
                t.tran_head_name LIKE %s
                AND tmh.id = 4        -- BANK
                AND tg.id = 3         -- 🔥 ONLY Deposit To Bank
...
def save_bank_deposit(request):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            tran_id = "BAN" + str(last_number + 1).zfill(9)
